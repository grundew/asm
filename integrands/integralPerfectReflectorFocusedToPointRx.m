function [X, f] = integralPerfectReflectorFocusedToPointRx(params, varargin)
...
for i = 1:nf
    ...
    w = 2*pi*f(i);
    k = w/c_F;
    fun = @(theta_z) integrand(theta_z, k, d1, d3, aTx, c_F, x0, r0);
...
function I = integrand(theta_z, k, d1, d3, aTx, c_F, x0, r0)
...
%% Assemble integrand
I = k.*q.*dispRx.*PhiTx.*Phase.*k.*p.^2;
end