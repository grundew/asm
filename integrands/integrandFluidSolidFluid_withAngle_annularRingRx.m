function I = integrandFluidSolidFluid_withAngle_annularRingRx(...
    gamma, f, aRx, aTx, c, rho, d, model, alpha)
...
% 
% This function is used for the case when the transmitter is a centre disc
% and the receiver is a outer annular ring. The inner radius of the
% receiver is assumed to be the radius of the transmitter.
% 
% Input:
...
q_rx = sin(gamma_rx);
kx = k*q_rx;
Wouter = besselj(1, kx*aRx)./kx/aRx;
Wouter(x==0) = 0.5;
Winner = besselj(1, x*aTx)./kx/aTx;
Winner(x==0) = 0.5;
PhiRx = 2*pi*(Wouter - Winner);
